<?php
	...
	class Presenter
	{
		...
		private function compile_tag($tag)
		{
			...
			//Пока, я зделал все что мог, что бы это хоть как то читалось.	//Будет время - введу define
		...
		public function get_tags($string)
		{
			...
			{		
				...
				if ((mb_strpos($value, '<!--') !== 0) and (preg_match_all($math, $value, $buflist)))
				{							
					foreach ($buflist[0] as $xvalue) 
					{
						$result[] = $xvalue;
					}
				}
		...
		//Метод возвращает true, если указанная строка содержит тегб оформленный в соответствии с правилами синтаксического оформления тегов шаблонизатора.
